/***** Elevator Controller (3층/외부 4버튼, 경유 시 잠시 정지, 타깃 확장 지원) **********
 * - 버튼 입력(층+방향) → 목적지(target) 결정 → 스텝 모터 이동 → 포토센서로 도착 판정
 * - 경유 규칙 (상행/하행 공통):
 *   (A) 타깃 확장: 진행방향과 같은 쪽으로 "현재 타깃을 넘어서는 호출"이 오면
 *       → 기존 타깃을 pending으로 밀고, 더 먼 쪽을 새로운 target으로 확장
 *       → 호출 방향(UP/DOWN)은 무시 (3F-DOWN도 ok)
 *   (B) 경로 사이 경유(기존 규칙): 진행방향과 일치 + 경로 ‘사이’인 호출은 즉시 경유
 * - 경유 도착 시: 500ms 잠시 정지(pause_active) → 타이머 만료 후 자동 재출발
 * - 대기열(pending)은 1칸만 사용(단순화). 필요 시 큐로 확장 가능.
 *************************************************************************************/
...
/* =========================================================================
 * 외부 버튼 처리(타깃 확장 + 경유 규칙)
 *  - 공통 무시: (buttonCall > 2) / (!moving && buttonCall==current)
 *  - 정지 상태: target=buttonCall; 즉시 출발 시도
 *  - 이동 상태:
 *     [A] 타깃 확장(방향 무시)
 *         · 상행: buttonCall > target  && pending==-1 → pending=target; target=buttonCall;
